...
test('line percentages affect only their own subtotal and round to cents', () => {
  ...
  assert.equal(percentMarkupAmount(6.25, '20.24'), 1.27);
  assert.equal(percentMarkupAmount(19.90, '25'), 4.98);
  assert.equal(percentMarkupAmount(-6.25, '20.24'), -1.27);
  assert.equal(percentMarkupAmount(6.25, '.5'), 0.03);
  assert.equal(percentMarkupAmount(6.25, ''), 0);
  assert.equal(percentMarkupAmount(100, -1), null);
  assert.equal(percentMarkupAmount(100, 'not a number'), null);
  assert.equal(percentMarkupAmount(100, '1e3'), null);
