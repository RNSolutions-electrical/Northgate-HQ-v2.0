...
export function MaterialAliases({item,permissions,onClose,onSaved}) {
 ...
 const [aliases,setAliases]=useState(item.item_aliases||[]),[alias,setAlias]=useState(''),[reason,setReason]=useState('');
 ...
 async function save(value,archived=false){
  ...
  if(!reason.trim()||!value.trim()){setError('Enter an alias and a reason for the change.');return;}
  lock.current=true;setBusy(true);setError('');setMessage('');
  try{
   const db=createSupabaseClient(await getToken({template:'supabase'}));
   const {data,error:rpcError}=await db.rpc('save_material_alias',{p_item_id:item.id,p_alias:value.trim(),p_archived:archived,p_reason:reason.trim()});
 ...
 <section className="card workspace-card inventory-location-setup" data-ng-ui-type="MODULE" data-ng-ui-name="Material Aliases">
  ...
  {canEdit&&<form onSubmit={e=>{e.preventDefault();save(alias);}}><fieldset disabled={busy} className="inventory-setup-grid">
   ...
   <label>Reason for alias change<input required value={reason} onChange={e=>setReason(e.target.value)}/></label>
  </fieldset><button className="primary-button" disabled={busy||!alias.trim()||!reason.trim()}>Add alias</button></form>}
  <ul className="inventory-alias-list">{aliases.map(row=><li key={row.id}><span>{row.alias}{row.archived_at?' (archived)':''}</span>{canEdit&&<button className={row.archived_at?'secondary-button':'secondary-button secondary-button--danger'} disabled={busy||!reason.trim()} onClick={()=>save(row.alias,!row.archived_at)}>{row.archived_at?'Restore':'Archive'} alias</button>}</li>)}</ul>
