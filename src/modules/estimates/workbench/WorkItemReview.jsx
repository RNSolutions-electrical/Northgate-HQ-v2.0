...
export function WorkItemReview({item, parent, data}) {
 ...
 return <section className="work-item-review" aria-label={`Verification details for ${item.name}`}>
  <div className="work-item-verification"><h3>Work item verification</h3>
  <dl className="review-facts">{[
   ['Entry',parent.name],['Location',parent.location],['Section',parent.section],['Drawing / sheet',parent.drawing],
   ['Entry internal notes',parent.internalNotes],['Work item',item.name],['Kind',item.kind],['Quantity',item.qty],['Status',item.status],['Internal notes',item.notes],
  ...
  <h3 className="review-components-heading">Components ({(item.lines || []).length})</h3>
  {(item.lines || []).map((line,index) => {
   ...
   return <div className="review-component" key={line.id || index}><strong className="component-reference">{componentReference(parent,item,index)}</strong><h4>{line.name || 'Unnamed component'}</h4><dl className="review-facts">{[
