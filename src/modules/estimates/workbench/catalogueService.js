...
export async function loadAssemblyLibrary(client){
 ...
 for(let from=0;;from+=500){
  const {data,error}=await client.from('assemblies')
   .select('id,name,unit,assembly_items(id,item_id,description,quantity,unit,unit_cost_snapshot,labor_rate_hrs_snapshot,note,archived_at)')
   .eq('is_library_item',true).is('archived_at',null).order('id').range(from,from+499);
  if(error)throw error;rows.push(...data);
  if(data.length<500)break;
 }
 return rows.map(a=>({id:a.id,name:a.name,qty:1,kind:'Assembly',lines:(a.assembly_items||[]).filter(l=>!l.archived_at).map(l=>({
  id:l.id,catalogueId:l.item_id||'',name:l.description,qty:Number(l.quantity),unit:l.unit||'EA',
  price:Number(l.unit_cost_snapshot),hours:Number(l.labor_rate_hrs_snapshot),stage:'Rough-in',fixed:false,notes:l.note||''
 }))}));
}
