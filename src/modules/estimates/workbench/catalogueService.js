...
export async function loadAssemblyLibrary(client){
 ...
 for(let from=0;;from+=500){
  const {data,error}=await client.from('assemblies')
   .select('id,name,division,unit,description,category,categories,updated_at,assembly_items(id,item_id,description,quantity,waste_percent,unit,unit_cost_snapshot,labor_rate_hrs_snapshot,note,archived_at,stage,fixed_quantity,price_missing,labor_missing,sort_order)')
   .eq('is_library_item',true).is('archived_at',null).order('id').range(from,from+499);
  if(error)throw error;rows.push(...data);
  if(data.length<500)break;
 }
 return rows.map(a=>({id:a.id,libraryId:a.id,division:a.division,categories:a.categories??(a.category?[a.category]:[]),updatedAt:a.updated_at,name:a.name,notes:a.description||'',qty:1,kind:'Assembly',status:'Not started',lines:(a.assembly_items||[]).filter(l=>!l.archived_at).sort((a,b)=>a.sort_order-b.sort_order).map(l=>({
