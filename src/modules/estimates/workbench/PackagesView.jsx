...
export function PackagesView({data,onPackage,onQuote,onAward,onlyPackage,locked=false,onArchive}){
 locked=locked||!!data.approvedAt;
 return <>
 ...
 {data.packages.filter(p=>!p.archivedAt&&(!onlyPackage||p.id===onlyPackage)).map(p=>{
  const entry=data.entries.find(e=>e.packageId===p.id);const quotes=data.quotes.filter(q=>q.packageId===p.id&&!q.archivedAt);
  return <section className="package-group" key={p.id}>
   ...
   <div className="actions"><button disabled={locked} aria-label={'Edit package '+p.name} onClick={()=>onPackage(p)}><Pencil size={16}/></button>{onArchive&&<button disabled={locked} onClick={()=>onArchive('package',p)}>Archive package</button>}<button disabled={locked} onClick={()=>onQuote(null,p.id)}><Plus size={16}/> Add vendor quote</button></div></div>
   {quotes.map(q=>{const awarded=p.awardedQuoteId===q.id;const value=itemPricing({quoteId:q.id,qty:1,lines:[],materialMarkupOverride:q.materialMarkupOverride},data);return <div className={'quote-row '+(awarded?'awarded':'')} key={q.id}>
    ...
    <div className="actions"><button disabled={locked} aria-label={'Edit quote from '+q.vendor} onClick={()=>onQuote(q,p.id)}><Pencil size={16}/></button>{onArchive&&<button disabled={locked} onClick={()=>onArchive('quote',q)}>Archive quote</button>}<button disabled={locked} onClick={()=>onAward(p.id,awarded?null:q.id)}>{awarded?<RotateCcw size={16}/>:<Check size={16}/>} {awarded?'Unaward':'Award quote'}</button></div>
   </div>;})}
