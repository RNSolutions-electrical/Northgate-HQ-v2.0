...
export function AttachedEstimates({jobId,changeOrderId,permissions,onUseQuote}){
 ...
 (async()=>{try{const db=createSupabaseClient(await getToken({template:'supabase'}));let query=db.from('estimate_workflow_handoffs').select('id,estimate_id,source_version,source_revision,destination,change_order_id,pricing,created_at').eq('job_id',jobId).order('created_at',{ascending:false});
