...
export function ServiceStageConsole() {
 ...
 const [stages,setStages]=useState([]),[selected,setSelected]=useState(''),[label,setLabel]=useState(''),[color,setColor]=useState('#FFF9C4'),[reason,setReason]=useState('');
 ...
 function choose(key){setSelected(key);const s=stages.find(s=>s.key===key);setLabel(s?.label || '');setColor(s?.background_color || '#FFF9C4');setReason('');setError('');setMessage('');}
 async function save(event) {
  event.preventDefault();if(busy)return;setBusy(true);setError('');setMessage('');
  try {
   const savedKey=await rpc('svc_save_stage',{p_key:selected || null,p_label:label,p_color:color,p_reason:reason,p_expected_updated_at:stages.find(s=>s.key===selected)?.updated_at || null});
   await reload();setSelected(savedKey);setReason('');setMessage('Stage saved. Refresh Service Calls to use the updated list and colors.');
 ...
 return <section className="developer-console-page svc-workspace" aria-label="Service call stages" {...uiElementAttributes('MODULE','Service Call Stages')}>
  ...
  <form onSubmit={save}><fieldset disabled={busy} className="svc-fieldset">
   <div className="svc-grid">
    ...
    <label className="svc-wide"><span>Audit reason</span><input type="text" required minLength={3} value={reason} onChange={e=>setReason(e.target.value)}/></label>
   </div>
   ...
   <button type="submit" className="primary-button" disabled={busy || !label.trim() || reason.trim().length<3}>{busy?'Saving…':selected?'Save stage':'Add stage'}</button>
  </fieldset></form>
