-- Approved by Ryan; applied through the controlled migration endpoint.
CREATE FUNCTION public.save_assembly_library(p_division text,p_assembly jsonb,p_estimate_id uuid DEFAULT NULL) RETURNS public.assemblies
...
DECLARE a public.assemblies; e public.estimates; component public.assembly_items;
 ...
 value numeric; k text; target_division text:=p_division;
BEGIN
 ...
 IF aid IS NULL THEN
  ...
  VALUES(target_division,btrim(p_assembly->>'name'),COALESCE(p_assembly->>'notes',''),'EA',auth.jwt()->>'sub',e.id)
  RETURNING * INTO a;
 ELSE
...
CREATE OR REPLACE FUNCTION public.save_workbench_assembly(
...
BEGIN
 IF p_estimate_id IS NULL THEN RAISE EXCEPTION 'Editable estimate required' USING ERRCODE='42501'; END IF;
 PERFORM public.save_assembly_library(p_division,p_assembly,p_estimate_id);
 RETURN public.save_estimate_workbench(p_estimate_id,p_division,p_document,p_expected_revision,p_catalogue_updates);
END $$;

-- Retain scoped visibility during soft archive; normal lists exclude archived rows.
DROP POLICY assemblies_read ON public.assemblies;
CREATE POLICY assemblies_read ON public.assemblies FOR SELECT TO authenticated USING(
...
CREATE FUNCTION public.archive_assembly_library(p_assembly_id uuid,p_expected_updated_at timestamptz,p_reason text)
RETURNS void LANGUAGE plpgsql SECURITY INVOKER SET search_path='' AS $$
DECLARE a public.assemblies;
BEGIN
 IF auth.jwt()->>'sub' IS NULL THEN RAISE EXCEPTION 'Sign in required' USING ERRCODE='28000'; END IF;
 IF COALESCE(btrim(p_reason),'')='' THEN RAISE EXCEPTION 'Archive reason required' USING ERRCODE='22023'; END IF;
 SELECT * INTO a FROM public.assemblies WHERE id=p_assembly_id AND archived_at IS NULL AND is_library_item FOR UPDATE;
 IF a.id IS NULL OR public.current_user_can_edit_division(a.division,'can_estimate') IS NOT TRUE THEN
  RAISE EXCEPTION 'Assembly unavailable or permission missing' USING ERRCODE='42501';
 END IF;
 IF a.updated_at IS DISTINCT FROM p_expected_updated_at THEN
  RAISE EXCEPTION 'Assembly changed. Refresh before archiving' USING ERRCODE='40001';
 END IF;
 UPDATE public.assemblies SET archived_at=clock_timestamp(),archived_by=auth.jwt()->>'sub',archive_reason=btrim(p_reason)
 WHERE id=a.id;
END $$;
REVOKE ALL ON FUNCTION public.archive_assembly_library(uuid,timestamptz,text) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.archive_assembly_library(uuid,timestamptz,text) TO authenticated;
