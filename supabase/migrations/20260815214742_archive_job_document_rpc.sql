...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Job document not found or already archived'
      USING ERRCODE = 'P0002';
  END IF;
  ...
  WHERE up.clerk_user_id = jwt_subject
    ...
    AND up.division = target_document.division
    AND COALESCE((
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'You do not have permission to archive this document'
      USING ERRCODE = '42501';
  END IF;
