-- Temporary development-only Pay App deletion under the existing
-- Developer Data Correction permission. Finalized records are deleted
-- latest-first and authoritative SOV billed totals are reconciled atomically.
...
CREATE OR REPLACE FUNCTION public.delete_unbilled_job_pay_application(
  p_pay_app_id uuid,
  p_reason text
)
RETURNS jsonb
...
SET search_path = public, pg_temp
AS $function$
  SELECT public.developer_delete_job_pay_application(p_pay_app_id,p_reason)
$function$;
