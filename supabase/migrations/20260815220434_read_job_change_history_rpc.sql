...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'You do not have permission to view this job history'
      USING ERRCODE = '42501';
  END IF;
  ...
  WHERE
    (
      cl.table_name = 'jobs'
      AND (
        cl.record_id = p_job_id::TEXT
        OR cl.before_data ->> 'id' = p_job_id::TEXT
        OR cl.after_data ->> 'id' = p_job_id::TEXT
        OR cl.before_data ->> 'job_id' = p_job_id::TEXT
        OR cl.after_data ->> 'job_id' = p_job_id::TEXT
        OR cl.before_data ->> 'owner_id' = p_job_id::TEXT
        OR cl.after_data ->> 'owner_id' = p_job_id::TEXT
      )
    )
    OR (
      cl.table_name IN ('job_buyout_lines', 'job_budget_lines', 'job_schedule_items')
      AND (
...
REVOKE ALL ON FUNCTION public.read_job_change_history(UUID, INTEGER) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.read_job_change_history(UUID, INTEGER) FROM anon;
