...
BEGIN
  ...
  IF COALESCE((caller_permissions ->> 'can_create_jobs')::BOOLEAN, FALSE) IS NOT TRUE THEN
    RAISE EXCEPTION 'Job creation permission is required' USING ERRCODE = '42501';
  END IF;
  ...
  INSERT INTO public.change_logs(user_id, user_name, table_name, record_id, action, note) VALUES (jwt_subject, COALESCE(caller.display_name, caller.email, jwt_subject), 'jobs', created_job.id::TEXT, 'create', 'Job ' || COALESCE(created_job.job_number, created_job.name) || ' created.');
  RETURN created_job;
END;
$$;
