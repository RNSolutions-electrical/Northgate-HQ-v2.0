-- Explicit reviewer access through the existing audited, stale-safe permission editor.
-- No role defaults, template contents, assignments or user grants change here.
...
BEGIN
  ...
  END IF;
  -- A Developer may receive this explicit reviewer capability; all other
  -- overrides and the template assignment must remain exactly as loaded.
  IF coalesce((public.permission_base_for_user(target.role,target.division,target.permission_overrides)->>'can_access_developer')::boolean,false)
     AND (p_template_id IS DISTINCT FROM prior_template
       OR (p_overrides - 'can_review_electrical_inspections') IS DISTINCT FROM
          (prior_overrides - 'can_review_electrical_inspections')) THEN
    RAISE EXCEPTION 'Only inspection reviewer permission may be changed for Developer users';
  END IF;
  prior_permissions := public.permission_template_base_for_user(p_user_id) || prior_overrides;
  IF p_template_id IS NOT DISTINCT FROM prior_template THEN
    NULL; -- Preserve unchanged assignment metadata.
...
REVOKE ALL ON FUNCTION public.save_user_permission_template(text,uuid,jsonb,jsonb,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.save_user_permission_template(text,uuid,jsonb,jsonb,text) TO authenticated;
