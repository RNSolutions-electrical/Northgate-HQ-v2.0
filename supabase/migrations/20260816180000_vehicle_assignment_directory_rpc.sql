CREATE OR REPLACE FUNCTION public.read_vehicle_assignment_directory(
...
BEGIN
  ...
  IF COALESCE((caller_permissions ->> 'can_manage_vehicles')::BOOLEAN, FALSE) IS NOT TRUE THEN
    RAISE EXCEPTION 'can_manage_vehicles permission is required to read vehicle assignments';
  END IF;
  ...
  WHERE p_vehicle_id IS NULL
     OR va.vehicle_id = p_vehicle_id
...
REVOKE ALL ON FUNCTION public.read_vehicle_assignment_directory(UUID, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.read_vehicle_assignment_directory(UUID, INTEGER) TO authenticated;
REVOKE EXECUTE ON FUNCTION public.read_vehicle_assignment_directory(UUID, INTEGER) FROM anon;

COMMENT ON FUNCTION public.read_vehicle_assignment_directory(UUID, INTEGER) IS
  'Read-only vehicle assignment directory for users with effective can_manage_vehicles permission. Keeps vehicle_assignments table ungranted to clients.';
