...
try {
 ...
 await db.exec(await readFile('supabase/migrations/20260914012059_assembly_categories.sql','utf8'));
 ...
 await save({name:'Reusable outlet',notes:'Library scope',categories:['Commercial','HVAC','commercial'],lines:[line]});
 ...
 assert.deepEqual(a.categories,['Commercial','HVAC']);
 ...
 assert.deepEqual(a.categories,['Commercial','HVAC']);
 await assert.rejects(save({name:'Bad category',categories:[42],lines:[line]}),/categories/);
