...
BEGIN
 ...
 blocked:=false;BEGIN PERFORM public.approve_workbench_estimate(w.estimate_id,'Rollback-only blank checklist');EXCEPTION WHEN invalid_parameter_value THEN blocked:=position('finalization checklist' in SQLERRM)>0;END;ASSERT blocked,'Blank checklist passed approval';
 blocked:=false;BEGIN PERFORM public.submit_estimate_for_review(w.estimate_id,w.revision,'job',j.id,NULL,NULL,'{}');EXCEPTION WHEN invalid_parameter_value THEN blocked:=position('finalization checklist' in SQLERRM)>0;END;ASSERT blocked,'Blank checklist passed submission';
 doc:=jsonb_set(doc,'{finalizationChecklist}','{"answers":{"available_fault_current":{"version":1,"status":"not_applicable"},"estimating_labor":{"version":1,"status":"included","values":{"hours":0}},"supervision_labor":{"version":1,"status":"excluded"}}}'::jsonb);
 w:=public.save_estimate_workbench(w.estimate_id,'Electrical',doc,w.revision,'[]');
 w2:=public.save_estimate_workbench(NULL,'Electrical',doc,NULL,'[]');
 PERFORM public.approve_workbench_estimate(w2.estimate_id,'Rollback-only complete checklist');
 ASSERT (SELECT status='approved' FROM public.estimates WHERE id=w2.estimate_id),'Complete zero/excluded checklist failed approval';
 ASSERT (SELECT workbench_document#>>'{finalizationChecklist,answers,estimating_labor,values,hours}'='0' FROM public.estimate_snapshots WHERE estimate_id=w2.estimate_id AND locked),'Zero consideration not preserved';
 RESET ROLE;
 ...
 'checklist blank rejected at both gates; complete zero/excluded answers approved and frozen; authenticated estimate save; CO draft and retry; stale rejection; new/existing job and service call; no budget/invoice/quote posting; exact cents; immutable source; unauthorized denial/RLS; all test data rolled back'));
END $$;
