...
const dir=process.argv[2],base=process.argv[3]||'https://rnsolutions.net',marker=process.argv[4]||'SAPPHIRE-ESTIMATING-WORKSPACE-20260916-001';
...
for(const value of ['set_document_tags','Use parent physical location','materials_summary'])assert.ok(main.includes(value),value);
...
for(const value of ['Finalization checklist','Submit for review','Approve estimate','finalizationChecklist','save_estimating_catalogue_material','manage_estimate_checklist','Add to catalogue','Reports / Export'])assert.ok(workbench.includes(value),value);
