...
## Final read-only Production preflight (2026-09-24, RYAN_NORTHGATE)

- Owner reported that the Staging badge no longer overlaps, the generated client Change Order PDF total matches the screen, and the “General Contractor Fee” wording looks good. These browser acceptance items are complete.
- Production Netlify site `northgate-hq-v2` still reports ready deploy `6ab2d90c269cd800087bc95c`. Production Supabase project `keogysnoukbendfkfjcn` remains healthy with 186 migration-ledger rows (latest version `20260922193652`), none of the four demo migration names, 45 Change Orders, $320,041.68 aggregate price, and $311,435.42 aggregate internal cost. No Production write was performed during this preflight.
- Production scheduled backups show a physical backup dated **2026-09-24 07:55:06 UTC**, in addition to the September 23 backup previously restored to a temporary isolated project for rehearsal. Point-in-time recovery is **not enabled**, and Supabase explicitly states database backups exclude Storage objects. The latest daily backup is a known recovery point but **not** an immediate pre-release recovery point; any intervening Production data changes would be lost in a full restore. Confirm that recovery-point tradeoff or establish an additional safe backup before promotion.
- The owner has confirmed the visible demo behavior but has not yet made an unambiguous, explicit Production promotion authorization after the preflight/recovery disclosure. Do not tag, migrate, merge to `main`, or deploy Production until that approval is obtained.
