-- Live function definitions captured read-only on 2026-09-15 for isolated checklist integration tests.
-- No production data. The new migration patches these existing transaction boundaries.
CREATE OR REPLACE FUNCTION public.approve_workbench_estimate(p_estimate_id uuid, p_approval_note text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
