...
CREATE TABLE public.user_permission_templates(user_id text PRIMARY KEY,template_id uuid,updated_by text,updated_at timestamptz DEFAULT now());
CREATE TABLE public.user_permission_overrides(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),user_id text,permission_flag text,granted boolean,granted_by_user_id text,granted_at timestamptz DEFAULT now(),reason text,is_active boolean DEFAULT true,review_note text,CONSTRAINT user_permission_overrides_permission_flag_check CHECK(permission_flag<>''));
