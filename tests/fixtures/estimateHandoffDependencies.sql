...
CREATE TABLE public.job_revenue_lines (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),job_id uuid NOT NULL,division text NOT NULL,created_at timestamptz DEFAULT now(),updated_at timestamptz DEFAULT now(),archived_at timestamptz,archived_by text,archive_reason text,sov_line text,description text NOT NULL,scheduled_value_amount numeric DEFAULT 0 NOT NULL,approved_change_amount numeric DEFAULT 0 NOT NULL,billed_to_date_amount numeric DEFAULT 0 NOT NULL,note text,created_by text,source_budget_line_id uuid,source_project_division_id uuid,source_original_budget_amount numeric DEFAULT 0,allocation_percent numeric,allocated_fee_amount numeric DEFAULT 0,billing_locked_at timestamptz,is_protected_financial boolean DEFAULT false);
CREATE TABLE public.job_pay_applications (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),job_id uuid NOT NULL,status text DEFAULT 'draft');
CREATE TABLE public.job_pay_application_lines (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),pay_application_id uuid,sov_line_id uuid,source_budget_line_id uuid);
CREATE TABLE public.change_order_allocations (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),budget_line_id uuid);
CREATE TABLE public.change_order_financial_postings (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),job_budget_line_id uuid);
CREATE TABLE public.change_order_sov_allocations (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),revenue_line_id uuid);
