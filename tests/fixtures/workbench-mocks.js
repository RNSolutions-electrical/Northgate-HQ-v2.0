...
window.workbenchFixture={calls:[],rows:[],failNext:false};
export function createSupabaseClient(){return {
 from(table){
  let start=0,end=999;
  const q={select:()=>q,eq:()=>q,is:()=>q,order:()=>q,range(a,b){start=a;end=b;return q;},
   then(resolve){return Promise.resolve({data:table==='items'?materialRows.slice(start,end+1):table==='assemblies'?[]:window.workbenchFixture.rows}).then(resolve);}};
  return q;
 },
 async rpc(name,args){
