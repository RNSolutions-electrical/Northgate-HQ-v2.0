import definitions from './estimateChecklistDefinitions.json';
...
window.workbenchFixture={calls:[],rows:[],snapshots:[],library:[],handoffs:[],definitions:structuredClone(definitions),failDefinitions:false,failNext:false};
if(location.search.includes('editing')){
 ...
 if(!location.search.includes('checklist'))doc.finalizationChecklist={answers:{available_fault_current:{status:'not_applicable',version:1,values:{}},estimating_labor:{status:'excluded',version:1,values:{}},supervision_labor:{status:'excluded',version:1,values:{}}}};
...
if(location.search.includes('checklist-review'))window.workbenchFixture.handoffs=[{id:'handoff-review',job_id:'job-1',change_order_id:null,source_version:1,source_revision:2,created_at:'2026-09-15T12:00:00Z',pricing:{total:100,lines:[]},checklist:{definitions:structuredClone(definitions),answers:{available_fault_current:{status:'not_applicable',version:1,values:{}},estimating_labor:{status:'included',version:1,values:{hours:0}},supervision_labor:{status:'excluded',version:1,values:{}}}}}];
export function createSupabaseClient(){return {
 ...
 async rpc(name,args){
  ...
  if(name==='approve_workbench_estimate'){
   ...
   snapshot.workbench_document.finalizationChecklist={...snapshot.workbench_document.finalizationChecklist,definitions:structuredClone(f.definitions),completedAt:snapshot.approved_at};
