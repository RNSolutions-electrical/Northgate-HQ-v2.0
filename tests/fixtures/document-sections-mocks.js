...
rows.forEach(row=>{row.organization_version=1;row.custom_tags=[];});
const jobs=[{id:'job1',job_type:'job',job_number:'J-001',name:'Fixture project',division:'Electrical'},{id:'call1',job_type:'service_call',service_call_number:'SC-001',name:'Fixture service',division:'Electrical'}];
...
export function createSupabaseClient(){return {
 ...
 rpc:async(name,args)=>{window.documentSectionsFixture.calls.push({name,args});if(name==='set_document_tags'){if(window.documentSectionsFixture.failNext){window.documentSectionsFixture.failNext=false;return {error:{message:'Document changed. Refresh before organizing.'}};}const row=rows.find(r=>r.id===args.p_id);row.department_tags=args.p_departments;row.custom_tags=args.p_tags;row.document_section=args.p_departments[0]||null;row.organization_version++;row.updated_at='2026-09-16';return {data:row};}throw Error('Unexpected RPC');},
