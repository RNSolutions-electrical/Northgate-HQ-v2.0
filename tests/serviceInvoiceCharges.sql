...
BEGIN
 ...
 invoice_data:=jsonb_build_object('invoice_number','__SVC_INVOICE_TEST','invoice_date','2026-09-14','due_date','2026-10-14',
   'total_revenue','100.00','sales_tax','7.25','sales_tax_percent','7.25','credit_card_percent','3','credit_card_fee','3.22','allocations',jsonb_build_array(
 ...
 OR (arow#>>'{financials,invoices,0,sales_tax}')::numeric+(brow#>>'{financials,invoices,0,sales_tax}')::numeric<>7.25 THEN RAISE EXCEPTION 'Allocation does not reconcile'; END IF;
 IF (arow#>>'{financials,invoices,0,credit_card_fee}')::numeric+(brow#>>'{financials,invoices,0,credit_card_fee}')::numeric<>3.22 THEN RAISE EXCEPTION 'Fee allocation does not reconcile'; END IF;
 IF (SELECT gross_profit FROM public.svc_call_financials WHERE job_id=a)<>30 THEN RAISE EXCEPTION 'Fee inflated profit'; END IF;
 FOREACH bad IN ARRAY ARRAY['NaN','Infinity','0.001','101','-1'] LOOP
  BEGIN
   PERFORM public.svc_post_invoice(gen_random_uuid(),invoice_data||jsonb_build_object('sales_tax_percent',bad,'invoice_number','__SVC_BAD_RATE'));
   RAISE EXCEPTION 'Bad rate accepted';
  EXCEPTION WHEN invalid_parameter_value THEN NULL; END;
 END LOOP;
 BEGIN
  PERFORM public.svc_post_invoice(gen_random_uuid(),invoice_data||'{"credit_card_fee":3.21}');
  RAISE EXCEPTION 'Wrong calculated fee accepted';
 EXCEPTION WHEN invalid_parameter_value THEN NULL; END;
 payment_data:=jsonb_build_object('request_id',gen_random_uuid(),'invoice_id',arow#>>'{financials,invoices,0,id}','amount',66.28,'payment_date','2026-09-14');
 PERFORM public.svc_save_commercial(a,'payment',payment_data,(arow->>'updated_at')::timestamptz);
 PERFORM public.svc_save_commercial(a,'payment',payment_data,(arow->>'updated_at')::timestamptz);
 SELECT value INTO arow FROM jsonb_array_elements(public.svc_read_calls()) WHERE value->>'id'=a::text;
 IF (SELECT outstanding FROM public.svc_call_financials WHERE job_id=a)<>0 THEN RAISE EXCEPTION 'Full payment including pass-through was not settled'; END IF;
